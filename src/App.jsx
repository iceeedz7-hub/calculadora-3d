...
const STORAGE_KEY = "calc3d_historico";
...
const C = {
  bg: "#07080a", surface: "#0e1014", surfaceHover: "#13161b",
  border: "#1c2028", borderHover: "#2a3040",
  accent: "#3b82f6", accentGlow: "rgba(59,130,246,0.15)", accentDim: "rgba(59,130,246,0.08)",
  gold: "#f0a445", goldGlow: "rgba(240,164,69,0.15)",
  text: "#e2e8f0", textMid: "#8896aa", textDim: "#3d4a5c",
...
function Card({ step, icon, title, delay, children }) {
  ...
  return (
    <div className="card" onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} style={{ background:C.surface, border:`1px solid ${h?C.borderHover:C.border}`, borderRadius:18, padding:"24px", transition:"all 0.25s", animationDelay:`${delay*0.07}s`, boxShadow:h?"0 8px 32px rgba(0,0,0,0.3)":"none" }}>
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      if (s) setHistorico(JSON.parse(s));
    } catch {}
  }, []);
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:680, margin:"0 auto", padding:"48px 20px 80px" }}>
        ...
        <div style={{ textAlign:"center", marginBottom:40 }}>
          <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:C.accentDim, border:"1px solid rgba(59,130,246,.2)", borderRadius:100, padding:"6px 16px", marginBottom:20 }}>
            ...
            <span style={{ fontSize:10, letterSpacing:"0.2em", color:C.accent, fontWeight:600 }}>FERRAMENTA DE PRECIFICAÇÃO</span>
          </div>
          <h1 style={{ fontSize:"clamp(40px,8vw,72px)", fontWeight:800, letterSpacing:"-0.03em", lineHeight:1, color:"#fff", marginBottom:8 }}>
            Calculadora<br />
            <span style={{ WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundImage:"linear-gradient(135deg,#3b82f6 0%,#818cf8 100%)", backgroundClip:"text" }}>3D Print</span>
          </h1>
        </div>
        ...
        <div style={{ display:"flex", gap:4, marginBottom:32, background:C.surface, border:`1px solid ${C.border}`, borderRadius:14, padding:4 }}>
          {[
            ...
            { id:"historico",   icon:"◷", label:`Histórico${historico.length>0?` (${historico.length})`:""}` },
          ].map(tab=>(
        ...
        {aba==="calculadora" && (
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            ...
            <Card step="00" icon="✦" title="Nome do Projeto" delay={0}>
              <div>
                <label style={sty.label}>Nome para identificar a cotação</label>
                <input type="text" name="nomeProjeto" value={form.nomeProjeto} onChange={handleChange}
                  placeholder="ex: Suporte de celular, Miniatura dragão..." style={sty.input}
                  onFocus={e=>Object.assign(e.target.style,sty.inputFocus)} onBlur={e=>Object.assign(e.target.style,sty.inputBlur)} />
              </div>
            </Card>
            ...
            <Card step="02" icon="⬡" title="Dimensões do Produto" delay={2}>
            ...
            <Card step="03" icon="◎" title="Filamento" delay={3}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                ...
                <NI label="Preço por kg" name="filamentKgPrice" value={form.filamentKgPrice} onChange={handleChange} placeholder="120" unit="R$" />
              </div>
            </Card>

            <Card step="04" icon="◷" title="Tempo de Produção" delay={4}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            <Card step="05" icon="⚡" title="Energia — 110v" delay={5}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                <NI label="Consumo da impressora" name="printerWatts" value={form.printerWatts} onChange={handleChange} placeholder="350" unit="W" hint="Creality Hi ≈ 300–400W médio" />
                <NI label="Tarifa kWh" name="kwhPrice" value={form.kwhPrice} onChange={handleChange} placeholder="0.75" unit="R$" hint="Média SP ≈ R$ 0,75" />
              </div>
            </Card>

            <Card step="06" icon="⚙" title="Desgaste da Impressora" delay={6}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
                <NI label="Valor da impressora" name="printerCost" value={form.printerCost} onChange={handleChange} placeholder="2200" unit="R$" />
                <NI label="Vida útil" name="printerLifespanHours" value={form.printerLifespanHours} onChange={handleChange} placeholder="5000" unit="h" />
                <NI label="Manutenção/mês" name="maintenanceMonthly" value={form.maintenanceMonthly} onChange={handleChange} placeholder="50" unit="R$" />
              </div>
            </Card>

            <Card step="07" icon="◈" title="Margem de Lucro" delay={7}>
            ...
            {result && (
              <div className="result-anim" style={{ background:C.surface, border:"1px solid rgba(240,164,69,.3)", borderRadius:20, overflow:"hidden", boxShadow:"0 0 48px rgba(240,164,69,.15)" }}>
                <div style={{ background:"linear-gradient(135deg,rgba(240,164,69,.12) 0%,rgba(240,164,69,.04) 100%)", borderBottom:"1px solid rgba(240,164,69,.2)", padding:"28px 32px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:16 }}>
                  <div>
                    <div style={{fontSize:10,letterSpacing:"0.2em",color:C.gold,fontWeight:600,marginBottom:6}}>PREÇO SUGERIDO DE VENDA</div>
                ...
                <div style={{padding:"24px 32px"}}>
                  <div style={{fontSize:10,letterSpacing:"0.15em",color:C.textDim,fontWeight:600,marginBottom:16}}>COMPOSIÇÃO DO CUSTO</div>
                  {[
                    ...
                    <div key={item.label} style={{marginBottom:12}}>
                      ...
                      <div style={{height:4,background:C.border,borderRadius:4,overflow:"hidden"}}>
                        <div style={{height:"100%",width:`${pct(item.value,result.subtotal)}%`,background:item.color,borderRadius:4,transition:"width 1s cubic-bezier(.16,1,.3,1)",boxShadow:`0 0 8px ${item.color}66`}} />
                      </div>
                    </div>
                  ))}
        ...
        {aba==="historico" && (
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            <div style={{position:"relative"}}>
              <span style={{position:"absolute",left:16,top:"50%",transform:"translateY(-50%)",color:C.textDim,fontSize:16}}>⌕</span>
              <input type="text" value={busca} onChange={e=>setBusca(e.target.value)} placeholder="Buscar projeto pelo nome..." style={{...sty.input,paddingLeft:44}}
                onFocus={e=>Object.assign(e.target.style,sty.inputFocus)} onBlur={e=>Object.assign(e.target.style,sty.inputBlur)} />
            </div>
            ...
            {filtrado.length===0?(
              <div style={{textAlign:"center",padding:"60px 20px"}}>
                ...
                <p style={{fontSize:12,color:C.textDim,marginTop:8}}>{busca?"Tente outro nome":"Calcule e clique em \"Salvar no Histórico\""}</p>
              </div>
            ):(
              filtrado.map(proj=>(
                  ...
                  {expandido===proj.id&&(
                    <div style={{borderTop:`1px solid ${C.border}`,padding:"16px 20px"}}>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
                        {[
                          {label:"Filamento",value:proj.filamentCost,color:"#3b82f6"},
                          {label:"Energia",value:proj.energyCost,color:"#8b5cf6"},
                          {label:"Desgaste",value:proj.wearCost,color:"#ec4899"},
                          {label:"Manutenção",value:proj.maintenanceCost,color:"#06b6d4"},
                        ].map(item=>(
        ...
        <p style={{textAlign:"center",color:C.textDim,fontSize:11,marginTop:40,fontFamily:"'JetBrains Mono',monospace"}}>valores são estimativas — ajuste conforme sua realidade</p>
      </div>
    </div>
  );
}
